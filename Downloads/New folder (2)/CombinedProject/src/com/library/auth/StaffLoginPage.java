    ...
    private void validateLogin() {
        ...
        try (java.sql.Connection conn = com.library.common.DatabaseConnection.getConnection();
            ...
            if (rs.next()) {
                ...
                } else if (dbRoleStr != null && dbRoleStr.equalsIgnoreCase("Staff")) {
                    role = UserSession.Role.STAFF;
                } else {
                    showMessage("Error", "Access Denied: You are not a staff member (Role: " + dbRoleStr + ")", false);
                    return;
                }

                UserSession.getInstance().login(userId, email, role);
                showMessage("Success", "Successful login as " + role, true);
                controller.navigateTo("Dashboard");
            } else {
                showMessage("Error", "Staff account not found. Please sign up.", false);
            }
