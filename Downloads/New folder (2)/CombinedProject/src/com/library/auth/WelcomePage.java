...
public class WelcomePage extends JPanel {
    ...
    private void createCardComponents() {
        ...
        // ===== Icon Label =====
        card.add(Box.createVerticalStrut(20)); // Compact top space
        iconLabel = new JLabel("📖");
        iconLabel.setAlignmentX(Component.CENTER_ALIGNMENT);
        iconLabel.setForeground(new Color(200, 160, 80));
    ...
    private void updateAllComponents(double scale) {
        ...
        int iconFontSize = (int) (55 * scale); // Scaled emoji size
        iconLabel.setFont(new Font("Segoe UI Emoji", Font.PLAIN, Math.max(35, iconFontSize)));
    ...
    private void updateVerticalSpacings(double scale) {
        card.removeAll();
        card.add(Box.createVerticalStrut((int) (20 * scale))); // Compact top
        card.add(iconLabel);
        card.add(Box.createVerticalStrut((int) (15 * scale)));

        card.add(welcomeLabel);
        card.add(Box.createVerticalStrut((int) (8 * scale)));
        ...
        card.add(buttonPanel);
        card.add(Box.createVerticalGlue());
        card.add(Box.createVerticalStrut((int) (10 * scale))); // Minimal footer buffer
        card.add(footerLabel);
    }
