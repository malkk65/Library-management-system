...
public class MemberBookView extends MemberBasePage {
    ...
    private JPanel createBookCard(Book book) {
        ...
        statusTag.setBackground(isAvailable ? new Color(220, 252, 245) : new Color(255, 240, 240));
        statusTag.setForeground(isAvailable ? new Color(0, 100, 50) : new Color(200, 40, 40));

        footer.add(statusTag, BorderLayout.WEST);

        if (isAvailable) {
            JButton borrowBtn = new JButton("Borrow");
            borrowBtn.setFont(new Font("Segoe UI", Font.BOLD, 12));
            borrowBtn.setBackground(new Color(139, 69, 19));
            borrowBtn.setForeground(Color.WHITE);
            borrowBtn.setFocusPainted(false);
            borrowBtn.setCursor(new Cursor(Cursor.HAND_CURSOR));
            borrowBtn.addActionListener(e -> {
                try {
                    int userId = com.library.common.UserSession.getInstance().getUserId();
                    if (userId == 0) {
                        UIUtils.showStyledMessage(this, "Error", "Please login again to borrow books.", false);
                        return;
                    }
                    BookService.getInstance().borrowBook(userId, book.getId());
                    UIUtils.showStyledMessage(this, "Success", "Book '" + book.getTitle() + "' borrowed successfully!",
                            true);
                    loadBooks(""); // Refresh view
                } catch (Exception ex) {
                    ex.printStackTrace();
                    UIUtils.showStyledMessage(this, "Error", "Could not borrow book: " + ex.getMessage(), false);
                }
            });
            footer.add(borrowBtn, BorderLayout.EAST);
        }
